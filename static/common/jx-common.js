/**
 * jx-common 首先要引入jQuery.js、jQuery.cookie.js、md5.js
...
window.jx_common = {
    ...
    appendParamToUrl: function (key, value, url) {
        var origUrl = url || document.referrer;
        if (!origUrl) {
            return;
        }
    ...
    checkPwd: function (pwd) {
        var exp = /^(?=.{6,16}$)(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z]+$/;
        return exp.test(pwd);
    },
    //输入2-15位字符(支持中文,数字,英文,下划线)
    checkNick: function (nick) {
        var exp = /^([a-zA-Z0-9_\u4e00-\u9fa5]){2,15}$/;
        return exp.test(nick);
    },
    //输入2-10位字符,支持中文、字母
    checkName: function (name) {
        var exp = /^([a-zA-Z\u4e00-\u9fa5]){2,10}$/;
    ...
    checkId: function (val) {
        var exp = /^((11|12|13|14|15|21|22|23|31|32|33|34|35|36|37|41|42|43|44|45|46|50|51|52|53|54|61|62|63|64|65)[0-9]{4})(([1|2][0-9]{3}(([0][1-9])|([1][0-2]))(([0][1-9])|([1-2][0-9])|([3][0-1]))[0-9]{3}[Xx0-9])|([0-9]{2}[0|1][0-9][0-3][0-9][0-9]{3}))$/;
        return exp.test(val);
    },
    checkJunRen: function (val) {
        var exp = /^[\\u4e00-\\u9fa5a-zA-Z0-9]{0,20}$/;
        return exp.test(val);
    },
    ...
    checkPassport: function (val) {
        var exp = /^1[45][0-9]{7}|G[0-9]{8}|P[0-9]{7}|S[0-9]{7,8}|D[0-9]+$/;
        return exp.test(val);
    },
    ...
    openConfig: function (configType, configOperation, configTitle) {
        ...
        if (configType == "FREE_TOUR_LIST") {
            var itemType = "FREE_LINE";
            var subjectValue = configOperation;
        ...
        if (configType == "AROUND_FUN_LIST") {
            var itemType = "FREE_LINE,TOUR_LINE";
            var subjectValue = configOperation;
            _self.openUrl("../../view/listpage/configlist.html?itemType=" + itemType + "&subjectValue=" + subjectValue + "&title=" + configTitle);
            return false;
        }
    ...
    openShop: function (id, fromType) {
        ...
        if (fromType == "listpage") {
            var shopUrl = "../../view/teamTravel/shop.html?id=" + id + '&type=' + fromType;
        } else {
            var shopUrl = "../../view/teamTravel/shop.html?id=" + id + "&shopname=" + fromType + '&type=childrenTravel';
        }
    ...
    downloadApp: function (targetAppUri, downloadChannel) {
        ...
        openapptimer = setTimeout(function () {
            var ch = downloadChannel ? downloadChannel : "wap";
    ...
    makeHdCh: function (defaultCh) {
        var _self = this;
        var ch = _self.getUrlParamByName("ch");
        if (!_self.isNull(ch)) {
    ...
    makeWeixinOpenId: function () {
        ...
        _self.loading("加载中...");
        var realUrl = encodeURIComponent(window.location.href);
        var jumpUrl = "http://" + document.domain + "/view/weixin/chjump.html?returnUrl=" + realUrl;
        var targetUrl = encodeURIComponent(jumpUrl);
        var appid = _wap_config.appid;
        var api1 = _wap_config.wxcenter + 'v1/openid/get/';
        var appUrl = api1 + appid + '?targetUrl=' + targetUrl;
    ...
    setTitle: function () {
        var _self = this;
        var title = _self.getUrlParamByName("title");
...
function wxBindPhone() {
    ...
    jx_common.bindPhone("绑定手机", function () {
        ...
        var yzm = $(".input-check-code").val();
        if (jx_common.isNull(yzm)) {
            jx_common.tip("请输入验证码");
            return false;
        }
        var openId = jx_common.getOpenIdByCookie();
        //用户绑定
        jx_ajax.post(jx__api, {
...
module.exports = window.jx_common;
